// --- 1) Prize ladder for each question (1–15)
const prizeAmounts = [
...
// --- 2) Your 15 hard-coded questions
const questions = [
...
// --- 4) Render the current question & its four answer buttons
function renderQuestion() {
  // Clear any previous status message
  statusEl.textContent = "";

  // Show the prize ladder for this question
  ...
  questions[current].choices.forEach((choiceText, idx) => {
...
function checkAnswer(selectedIdx) {
  ...
  if (selectedIdx === correctIdx) {
    ...
    if (current < questions.length) {
      ...
      // Winner!
      statusEl.textContent = `🎉 You’ve won \$${prizeAmounts[prizeAmounts.length - 1]}!`;
      questionEl.textContent = "";
      answersEl.innerHTML = "";
    }
  } else {
    // Game over, show final safe-level prize
    statusEl.textContent = `❌ Wrong! You leave with \$${prizeAmounts[Math.floor(current/5)*5]}.`;
    questionEl.textContent = "";
    answersEl.innerHTML = "";
  }
}

// --- 6) Lifelines (after core logic is defined!)
...
// --- 7) Start the game for the first time
renderQuestion();
